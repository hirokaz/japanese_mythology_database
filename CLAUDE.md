...
8. **サブタスク単位 PR**: issue 本文に `- [ ] **S<NN>** ...` 形式のサブタスクがある場合、**サブタスク 1 つ = PR 1 本 = マージ 1 回** で進める。各サブタスク完了時に issue 本文のチェックボックスを ✓ に更新し、全サブタスク完了で issue を close。
9. **タイムアウト対策**: 大量データ生成系 issue は必ずサブタスクに分解する。1 PR の作業時間目安は 4 時間以内、上限 8 時間。詳細手順は `docs/project/15_workflow.md` を参照。
